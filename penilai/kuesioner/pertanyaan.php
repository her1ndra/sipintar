...
$stmt = $pdo->prepare('SELECT * FROM kuesioner WHERE id_kuesioner = ? AND id_user_pembuat = ?');
$stmt->execute([$idKuesioner, $user['id_user']]);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare('SELECT COALESCE(MAX(nomor_urut),0)+1 n FROM pertanyaan_kuesioner WHERE id_kuesioner = ?');
    $stmt->execute([$idKuesioner]);
    $nomor = $stmt->fetch()['n'];

    $pdo->prepare(
        'INSERT INTO pertanyaan_kuesioner (id_kuesioner, nomor_urut, teks_pertanyaan, bobot) VALUES (?, ?, ?, ?)'
...
$stmt = $pdo->prepare('SELECT * FROM pertanyaan_kuesioner WHERE id_kuesioner = ? ORDER BY nomor_urut');
$stmt->execute([$idKuesioner]);
