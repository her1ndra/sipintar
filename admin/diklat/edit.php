...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    } elseif ($idPegawai <= 0 || $metode === '' || !in_array($prioritas, ['Tinggi', 'Sedang', 'Rendah'], true) || !in_array($status, ['Diusulkan', 'Disetujui', 'Terlaksana', 'Ditolak'], true) || $tahun < 2000) {
        setFlash('error', 'Data kebutuhan diklat belum lengkap atau tidak valid.');
    } else {
        $pdo->prepare('UPDATE kebutuhan_diklat SET id_pegawai = ?, id_kesenjangan = ?, id_diklat = ?, metode_pengembangan = ?, prioritas = ?, status = ?, tahun_rencana = ?, catatan = ? WHERE id_kebutuhan = ?')
            ->execute([$idPegawai, $idKesenjangan, $idDiklat, $metode, $prioritas, $status, $tahun, $catatan, $id]);
...
<div class="mb-3"><label class="form-label">Diklat yang dibutuhkan</label><select name="id_diklat" class="form-select"><option value="">-- belum ditentukan --</option>
<?php foreach ($diklatList as $diklat): ?><option value="<?= (int) $diklat['id_diklat'] ?>" <?= (int) ($data['id_diklat'] ?? 0) === (int) $diklat['id_diklat'] ? 'selected' : '' ?>><?= htmlspecialchars($diklat['nama_diklat']) ?></option><?php endforeach; ?></select></div>
