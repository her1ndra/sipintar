...
<form method="post" enctype="multipart/form-data" class="bg-white p-4 rounded shadow-sm">
    <div class="mb-3">
        ...
        <?php foreach ($details as $item): ?><div class="form-control bg-light mb-2"><?= htmlspecialchars($item['kompetensi']) ?></div><?php endforeach; ?>
    </div>
    <div class="mb-3">
        <label class="form-label">Daftar Pertanyaan</label>
        <?php foreach ($details as $i => $item): ?><div class="form-control bg-light mb-2"><?= $i + 1 ?>. <?= htmlspecialchars($item['isi_penilaian']) ?></div><?php endforeach; ?>
    </div>
    <div class="mb-3">
        <label class="form-label">Bukti Kompetensi</label>
        <input type="file" name="file_bukti" class="form-control" accept=".jpg,.jpeg,.png">
        <small class="text-muted">Satu file JPG/PNG, maksimal 5 MB.</small>
